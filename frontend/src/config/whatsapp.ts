...
export const WHATSAPP_CONFIG = {
   ...
   */
  rawNumber: (safeEnv.VITE_WHATSAPP_NUMBER as string | undefined)?.trim() || '8374404536',
...
export function validateProviderWhatsAppNumber(input = PROVIDER_WHATSAPP_NUMBER): {
  ...
  if (nationalDigits.length !== 10) {
    return {
      ...
      errorMessage: `Provider WhatsApp number (${raw}) is incomplete. It contains ${nationalDigits.length} digits instead of the required 10 digits for an Indian mobile number. Missing digits cannot be silently guessed or invented.`,
    };
  }
 ...
 * suitable for standard wa.me URLs (e.g. "918374404536").
 */
export function normalizeWhatsAppNumber(
  ...
  // Remove leading single zero if present (e.g. 08374404536 -> 8374404536)
  if (digits.startsWith('0') && digits.length > 10) {
    digits = digits.substring(1);
  }

  // If already prefixed with country code (e.g., 918374404536)
  if (digits.startsWith(cc) && digits.length === cc.length + 10) {
    return digits;
  }

  // If 10-digit number without country code (e.g., 8374404536)
  if (digits.length === 10) {
    return `${cc}${digits}`;
  }
 ...
 * Formats 8374404536 -> "+91 83744 04536".
 */
export function formatWhatsAppDisplayNumber(
  ...
  // If standard 10-digit number, format as +91 83744 04536
  if (digits.length === 10) {
...
export function validateAndNormalizeCustomerWhatsApp(phoneInput: string): {
  ...
  // Strip leading 0 if 11 digits (e.g., 08374404536 -> 8374404536 -> 918374404536)
