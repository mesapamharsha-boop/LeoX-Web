...
export function resolveApiBase(): string {
  ...
  if (typeof window !== 'undefined') {
    ...
    // If on a remote deployment (like *.run.app) and envUrl points to localhost,
    // ignore localhost to prevent mixed-content and connection refused errors
    if (!isLocalhost && (envUrl.includes('localhost') || envUrl.includes('127.0.0.1'))) {
      return '/api';
    }

    // If served from port 3000 (backend hosting Vite/SPA directly), relative '/api' is always same-origin
    if (isLocalhost && (window.location.port === '3000' || !window.location.port)) {
      return '/api';
    }

    // If running separate Vite dev server on port 5173 without explicit VITE_API_URL
    if (isLocalhost && window.location.port === '5173' && !envUrl) {
