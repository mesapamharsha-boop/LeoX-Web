...
export interface WhatsAppButtonProps {
  ...
  /** Custom button text */
  label?: string;
  /** Whether to display the formatted phone number alongside or below */
  showNumber?: boolean;
...
export const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
  ...
  const displayNumber = formatWhatsAppDisplayNumber(number);
  ...
  if (variant === 'icon') {
    return (
      <a
        ...
        aria-label={`Chat on WhatsApp at ${displayNumber}`}
        title={`Chat on WhatsApp (${displayNumber})`}
  ...
  if (variant === 'pill') {
    return (
      ...
      >
        ...
        <span>{label}</span>
        {showNumber && <span className="opacity-80">({displayNumber})</span>}
      </a>
    );
  }
  ...
  if (variant === 'outline') {
    return (
      ...
      >
        ...
        <span>{label}</span>
        {showNumber && <span className="text-xs opacity-75 font-normal">({displayNumber})</span>}
      </a>
    );
  }
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {showNumber && (
        <span className="text-white/90 font-mono text-[11px] tracking-normal font-normal">
          {displayNumber}
        </span>
      )}
    </a>
